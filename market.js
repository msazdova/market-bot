...
const fetchOrders = async () => {
    ...
    try {
        ...
        console.log('An error occured whilst trying to request Orderbook API: ', err);
        orders = [];
    }
...
const placeOrders = async (highestBid, lowestAsk) => {
    for (let i = 0; i < config.market.numberOfOrders; i++) {
        const { orderPrice: bidPrice, orderAmount: bidAmount } = await makeOrder(highestBid, assets.USD);
        ...
        console.log("PLACE BID @ " + bidPrice + " " + bidAmount.dividedBy(bidPrice));

        const { orderPrice: askPrice, orderAmount: askAmount } = await makeOrder(lowestAsk, assets.ETH);
        ...
        console.log("PLACE ASK @ " + askPrice + " " + askAmount.dividedBy(askPrice));
    }
}

const updateOrders = async (bidOrders, askOrders) => {
    await Promise.all(bidOrders.map(order => {
    ...
    await Promise.all(askOrders.map(order => {
